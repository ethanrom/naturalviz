...
def tab1():
    ...
    st.write(
        github_link + "&nbsp;&nbsp;&nbsp;" + huggingface_link, unsafe_allow_html=True
    )

    st.markdown(
        "<p style='font-size: 14px; color: #777;'>Disclaimer: This app is a proof-of-concept and may not be suitable for real-world decisions. During the Hackthon period usage information are being recorded using Langsmith</p>",
...
def tab3():
    ...
    if st.session_state.error and st.button("Retry"):
        with st.spinner("🤔 Thinking..."):
            result2 = retry_generate_plot(
                question, st.session_state.error, st.session_state.generated_code
            )
